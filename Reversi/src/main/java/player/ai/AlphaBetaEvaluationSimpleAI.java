...
public class AlphaBetaEvaluationSimpleAI extends Player {
    ...
    protected EvalResult eval(Board board, int restDepth, Stone stone, Evaluation evaluation, double alpha, double beta) {
        if (restDepth == 0)
        ...
        for (int y = 1; y <= Board.HEIGHT; ++y) {
            for (int x = 1; x <= Board.WIDTH; ++x) {
                if (board.isPuttable(x, y, stone)) {
                    ...
                    b.put(x, y, stone);
                    double score = -eval(b, restDepth - 1, stone.flip(), e, -beta, -Math.max(alpha, maxScore)).getScore();
                    if (maxScore < score) {
                        maxScore = score;
                        p = new Position(x, y);
                    }
        ...
        if (didPlayed) {
            ...
            double score = -eval(board, restDepth - 1, stone.flip(), evaluation, -beta, -alpha).getScore();
