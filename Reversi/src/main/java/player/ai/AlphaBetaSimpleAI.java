...
public class AlphaBetaSimpleAI extends Player {
    ...
    private EvalResult eval(Board board, int restDepth, Stone stone, int scoreSum, double alpha, double beta) {
        if (restDepth == 0)
        ...
        if (didPlayed) {
            ...
            double score = -eval(board, restDepth - 1, stone.flip(), scoreSum, -beta, -alpha).getScore();
